...
// log in.  We first find the user model in MySQL that matches the submitted email,
...
passport.use(new LocalStrategy({ usernameField: 'email' }, (email, password, done) => {
    UserModel.findByEmail(email).then(user => {
        ...
        UserModel.comparePassword(user.password, password, (err, isMatch) => {
...
function signup({ email, password, req }, done) {
    const user = { email, password };
    if (!email || !password) { throw new Error('You must provide an email and password.'); }
    return new Promise((resolve, reject) => {
        UserModel.findByEmail(email)
            .then(existingUser => {
                if (existingUser) { reject("Email in use."); }
            });
